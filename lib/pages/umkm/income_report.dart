  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Laporan', textAlign: TextAlign.center,style: whiteTextStyle.copyWith(fontSize: MediaQuery.of(context).size.width * 0.05, fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: Center(
        child: Column(
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        child: ToggleButtons(
          ...
          //highlightColor: Color(0xFF06C474),
          renderBorder: false,
          children: <Widget>[
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text('Pendapatan', style: TextStyle(fontFamily: 'Nunito', fontSize: 16, fontWeight: FontWeight.bold),),),
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text('Aktivitas', style: TextStyle(fontFamily: 'Nunito', fontSize: 16, fontWeight: FontWeight.bold),),),
          ],
          onPressed: (int newIndex){
            setState(() {
              for(int index = 0; index < _isSelect.length; index++){
                if(index == newIndex){
                  _isSelect[index] = true;
                }else{
                  _isSelect[index] = false;
                }
              }

              if(newIndex == 0){
                _selectedButton = 'Pendapatan';
              } else {
                _selectedButton = 'Aktivitas';
              }
            });
          },
        )
